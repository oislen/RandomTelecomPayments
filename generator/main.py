...
def main(input_params_dict: dict):
    ...
    if input_params_dict["n_itr"] > 1:
        ...
        results = multiprocess(
            func=gen_random_telecom_data, args=args, ncpu=os.cpu_count()
        )
    else:
    ...
    user_data = pd.concat(
        objs=[result["user_data"] for result in results], axis=0, ignore_index=True
    )
    trans_data = pd.concat(
        objs=[result["trans_data"] for result in results], axis=0, ignore_index=True
    )
    ...
    data_fdirs = [
        os.path.dirname(cons.fpath_randomtelecomtransdata),
        os.path.dirname(cons.fpath_randomtelecomusersdata),
    ]
    ...
    # write data to disk
    logging.info(
        f"Writing intermediate user level random telecoms data to: {cons.fpath_randomtelecomusersdata}"
    )
    logging.info(
        f"Writing output trans level random telecoms data to: {cons.fpath_randomtelecomtransdata}"
    )
