...
class Device:
    ...
    def __init__(
        ...
        self.device_hashes_cnts_dict = gen_idhash_cnt_dict(
            idhash_type="hash", n=self.n_device_hashes, lam=self.lam, power=self.power
        )
        self.device_hashes = list(self.device_hashes_cnts_dict.keys())
        self.device_hashes_props_dict = cnt2prop_dict(
            idhashes_cnts_dict=self.device_hashes_cnts_dict
        )
        self.device_hashes_type_dict = self.gen_device_types(
            device_hashes=self.device_hashes, fpath_smartphones=self.fpath_smartphones
        )
        self.device_shared_idhash_map_dict = gen_shared_idhashes(
            idhashes=self.device_hashes,
            prop_shared_idhashes=self.prop_shared_device_hashes,
        )
    ...
    def gen_device_types(
        ...
        smartphone_data = pd.read_csv(
            fpath_smartphones, usecols=["model", "rating", "os"]
        )
        smartphone_data = smartphone_data.loc[
            smartphone_data["os"] == "android", :
        ].copy()
        # derive a proportion popularity from the ratings(TODO: expand this to be average rating per mobile phone brand)
        smartphone_data["rating"] = smartphone_data["rating"].fillna(
            value=smartphone_data["rating"].mean()
        )
        smartphone_data["popularity"] = (
            smartphone_data["rating"] / smartphone_data["rating"].sum()
        )
        # randomly choose different device types
        device_types = list(
            np.random.choice(
                a=smartphone_data["model"].to_list(),
                size=len(device_hashes),
                replace=True,
                p=smartphone_data["popularity"].to_list(),
            )
        )
        # return device hashes and types
        device_hashes_type_dict = dict(zip(device_hashes, device_types, strict=False))
