...
def gen_trans_status(
    ...
    if pd.notna(series["card_hash"]):
        ...
        if (
            rejection_rates_dict["country_code_trans_reject_rate_dict"][
                np.random.choice(
                    a=series[country_code_columns].dropna().to_list(), size=1
                )[0]
            ]
            >= random.uniform(0, 1) / rejection_scaling_factor
        ):
            error_code = np.random.choice(
                a=list(cons.data_model_rejection_codes_fraud.keys()),
                p=list(cons.data_model_rejection_codes_fraud.values()),
                size=1,
            )[0]
        # add rejections based on domain frequencies
        elif (
            rejection_rates_dict["domain_email_trans_reject_rate_dict"][
                series["email_domain"]
            ]
            >= random.uniform(0, 1) / rejection_scaling_factor
        ):
            error_code = np.random.choice(
                a=list(cons.data_model_rejection_codes_authentication.keys()),
                p=list(cons.data_model_rejection_codes_authentication.values()),
                size=1,
            )[0]
        # add rejections based on inconsistent country codes
        elif (
            cons.data_model_inconsistent_country_codes_rejection_rate[
                series[country_code_columns].dropna().nunique()
            ]
            >= random.uniform(0, 1) / rejection_scaling_factor
        ):
            error_code = np.random.choice(
                a=list(cons.data_model_rejection_codes_connection.keys()),
                p=list(cons.data_model_rejection_codes_connection.values()),
                size=1,
            )[0]
        # add rejections based on shared ips, cards and devices
        elif (
            pd.notna(series["device_hash"])
            and rejection_rates_dict["shared_devices_reject_rate_dict"][
                series["device_hash"]
            ]
            >= random.uniform(0, 1) / rejection_scaling_factor
        ):
        ...
        # add rejections based on counts of devices, ips and cards
            ...
            rejection_rates_dict["count_devices_reject_rate_dict"][series["userid"]]
            >= random.uniform(0, 1) / rejection_scaling_factor
        ):
            error_code = np.random.choice(
                a=list(cons.data_model_rejection_codes_user.keys()),
                p=list(cons.data_model_rejection_codes_user.values()),
                size=1,
            )[0]
        elif (
            rejection_rates_dict["count_ips_reject_rate_dict"][series["userid"]]
            >= random.uniform(0, 1) / rejection_scaling_factor
        ):
            error_code = np.random.choice(
                a=list(cons.data_model_rejection_codes_connection.keys()),
            ...
            rejection_rates_dict["count_cards_reject_rate_dict"][series["userid"]]
            >= random.uniform(0, 1) / rejection_scaling_factor
        ):
            error_code = np.random.choice(
                a=list(cons.data_model_rejection_codes_funds.keys()),
                p=list(cons.data_model_rejection_codes_funds.values()),
                size=1,
            )[0]
        # otherwise return successful status
        else:
            successful_status = {
                key: cons.data_model_transaction_status[key]
                for key in ["Successful", "Pending"]
            }
            successful_probs = [
                value / sum(successful_status.values())
                for value in successful_status.values()
            ]
            status = np.random.choice(
                a=list(successful_status.keys()), size=1, p=successful_probs
            )[0]
            error_code = np.nan
    else:
