...
def join_idhashes_dict(
    data: pd.DataFrame,
    idhashes_dict: dict[str | int, object],
    idhash_key_name: str,
    idhash_val_name: str,
):
    """
    ...
    """
    idhashes_df = pd.DataFrame(
        list(idhashes_dict.items()), columns=[idhash_key_name, idhash_val_name]
    )
    idhashes_join = pd.merge(
        left=data, right=idhashes_df, on=idhash_key_name, how="left"
    )
