...
def gen_trans_rejection_rates(
    ...
    countrycrimeindex = pd.read_csv(
        fpath_countrycrimeindex, usecols=["country_code", "crime_index"]
    )
    ...
    domain_email = (
        pd.read_csv(fpath_email_domain, encoding="utf-8")
        .groupby(by=["email_domains"], as_index=False)
        .agg({"probability": "mean"})
    )
    domain_email["trans_reject_rate"] = (
        domain_email["probability"] / domain_email["probability"].sum()
    )
    domain_email_trans_reject_rate_dict = domain_email.set_index("email_domains")[
        "trans_reject_rate"
    ].to_dict()
    ...
    rejection_rates_dict["count_devices_reject_rate_dict"] = (
