...
def gen_random_entity_counts(
    ...
    random_entity_counts["uid"] = np.random.choice(
        a=user_obj.user_ids, size=user_obj.n_user_ids, replace=False
    )
    # randomly simulate the number of entities per user
    for object_type in cons.object_types:
        random_entity_counts[f"n_{object_type}s"] = gen_random_poisson_power(
            lam=cons.data_model_poisson_params[object_type]["lambda"],
            size=user_obj.n_user_ids,
            power=cons.data_model_poisson_params[object_type]["power"],
        )
    # scale n transactions by
    random_entity_counts["n_transactions"] = (
        random_entity_counts["n_transactions"] * transaction_timescale
    ).astype(int)
