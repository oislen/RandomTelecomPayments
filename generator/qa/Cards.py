...
class Cards:
    ...
    def unique_card_types(self):
        """ """
        nunique_card_types_per_card = self.data.groupby(
            by=["card_hash"], as_index=False, dropna=False
        ).agg({"card_type": "nunique"})
        card_type_payment_totals = self.data.groupby(
    ...
    def unique_country_codes(self):
        """ """
        nunique_country_codes_per_card = self.data.groupby(
            by=["card_hash"], as_index=False, dropna=False
        ).agg({"card_country_code": "nunique"})
        card_country_code_payment_totals = (
            self.data.assign(
                card_country_code=self.data["card_country_code"].notnull().astype(int)
            )
            .groupby(
                by=["card_country_code", "transaction_payment_method"],
                as_index=False,
                dropna=False,
            )
            .agg({"transaction_hash": "size", "transaction_amount": "sum"})
        ...
        if self.show_plots:
            # plot distribution
            sns.histplot(
                data=nunique_country_codes_per_card, x="card_country_code", bins=20
            )
        if self.show_logs:
            # show logs
