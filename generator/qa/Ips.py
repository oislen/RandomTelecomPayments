...
class Ips:
    ...
    def unique_country_codes(self):
        """ """
        nunique_country_codes_per_ip = self.data.groupby(
            by=["ip_hash"], as_index=False, dropna=False
        ).agg({"ip_country_code": "nunique"})
        ip_country_codes_totals = self.data.groupby(
            by=["ip_country_code"], as_index=False, dropna=False
        ).size()
        # test assertions
        ...
        if self.show_plots:
            # plot distribution
            sns.histplot(
                data=nunique_country_codes_per_ip, x="ip_country_code", bins=20
            )
        if self.show_logs:
            # show logs
