...
class Uids:
    ...
    def unique_user_ids(self):
        """ """
        nunique_user_ids_per_uid = (
            self.data.groupby(by="uid", dropna=False, as_index=False)
            .agg({"userid": "nunique"})
    ...
    def unique_names(self):
        ...
        nunique_names_per_uid = (
            tmp_data.groupby(["userid"], dropna=False, as_index=False)
            .agg({"fullname": "nunique"})
    ...
    def unique_reg_dates(self):
        """ """
        nunique_reg_date_per_uid = (
            self.data.groupby(by="uid", dropna=False, as_index=False)
            .agg({"registration_date": "nunique"})
            .sort_values(by=["registration_date"])
        )
        # test assertions
    ...
    def nunique_reg_countries(self):
        """ """
        nunique_reg_country_per_uid = (
            self.data.groupby(by="uid", dropna=False, as_index=False)
            .agg({"registration_country_code": "nunique"})
            .sort_values(by=["registration_country_code"])
        )
        # test assertions
        ...
        if self.show_plots:
            # plot distribution
            sns.histplot(
                data=nunique_reg_country_per_uid, x="registration_country_code", bins=20
            )

    def unique_email_domains(self):
        """ """
        nunique_email_domains_per_uid = (
            self.data.groupby(by="uid", dropna=False, as_index=False)
            .agg({"email_domain": "nunique"})
    ...
    def unique_apps(self):
        """ """
        nunique_apps_per_uid = (
            self.data.groupby(by="uid", dropna=False, as_index=False)
            .agg({"application_hash": "nunique"})
            .sort_values(by=["application_hash"])
        )
        # test assertions
    ...
    def unique_transactions(self):
        ...
        nunique_ips_per_uid = (
            self.data.groupby(by="uid", dropna=False, as_index=False)
            .agg({"transaction_hash": "nunique"})
            .sort_values(by=["transaction_hash"])
        )
        transaction_payment_rel = (
            self.data.assign(
                transaction_hash=self.data["transaction_hash"].notnull().astype(int)
            )
            .groupby(
                by=["transaction_hash", "transaction_payment_method"],
                as_index=False,
                dropna=False,
            )
        ...
        assert (
            transaction_payment_rel.loc[
                transaction_payment_rel["transaction_payment_method"] == "Card",
                "transaction_amount",
            ].iloc[0]
            > 0
        )
        assert (
            transaction_payment_rel.loc[
                transaction_payment_rel["transaction_payment_method"] == "Points",
                "transaction_amount",
            ].iloc[0]
            > 0
        )
        assert (
            transaction_payment_rel.loc[
                transaction_payment_rel["transaction_payment_method"] == "Wallet",
                "transaction_amount",
            ].iloc[0]
            > 0
        )
        assert (
            transaction_payment_rel.loc[
                transaction_payment_rel["transaction_payment_method"].isnull(),
                "transaction_amount",
            ].iloc[0]
            == 0
        )
