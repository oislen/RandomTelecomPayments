...
async def get_api(
    n_users: Annotated[
        int, Query(title="Number of Users", description="The number of users")
    ] = cons.default_n_users,
    use_random_seed: Annotated[
        int,
        Query(
            title="Use Random Seed", description="The random seed to use", ge=0, le=1
        ),
    ] = cons.default_use_random_seed,
    n_itr: Annotated[
        int,
        Query(
            title="Number of Iterations", description="The number of iterations", ge=1
        ),
    ] = cons.default_n_itr,
    n_applications: Annotated[
        int,
        Query(
    ...
    ] = cons.default_n_applications,
    registration_start_date: Annotated[
        str,
        Query(
            title="Registration Start Date",
            description="The registration start date in YYYY-MM-DD format",
        ),
    ] = cons.default_registration_start_date,
    registration_end_date: Annotated[
        str,
        Query(
            title="Registration End Date",
            description="The registration end date in YYYY-MM-DD format",
        ),
    ] = cons.default_registration_end_date,
    transaction_start_date: Annotated[
        str,
        Query(
            title="Transaction Start Date",
            description="The transaction start date in YYYY-MM-DD format",
        ),
    ] = cons.default_transaction_start_date,
    transaction_end_date: Annotated[
        str,
        Query(
            title="Transaction End Date",
            description="The transaction end date in YYYY-MM-DD format",
        ),
    ] = cons.default_transaction_end_date,
):
    """
...
async def post_api(body: dict[str, object] = {}):
    """
    Generate random telecom payments data based on user-defined parameters.
