...
system_email = [
    {
        "text": system_email_prompt,
    }
]

first_name_prompt = (
    'Generate {n_data_points} first names for people from the country "{country}"'
)
last_name_prompt = (
    'Generate {n_data_points} last names for people from the country "{country}"'
)
...
def invoke_bedrock(
    ...
    # call bedrock model
    formatted_prompt = data_point_prompt_dict[data_point][0].format(
        n_data_points=n_data_points, country=country
    )
    ...
    gen_dataframe = (
        pd.Series(gen_data_list, name=data_point)
        .drop_duplicates()
        .to_frame()
        .reset_index()
    ...
    def standardise_text_lambda(x):
        return unidecode.unidecode(" ".join(x.strip().split())) if not pd.isna(x) else x

    gen_country_dataframe[data_point] = gen_country_dataframe[data_point].apply(
        lambda x: standardise_text_lambda(x)
    )
    ...
    # concatenate results
    gen_country_dataframe = pd.concat(
        objs=[gen_country_dataframe, tmp_gen_country_dataframe],
        axis=0,
        ignore_index=True,
    )
    # deduplicate data
    ...
    agg_dict = {"rank": "mean"}
    gen_country_dataframe = (
        gen_country_dataframe.dropna()
...
def main(bedrock, model_id, data_point, fpath_dict, run_bedrock=False):
    ...
    countrieseurope = pd.read_csv(
        cons.fpath_countries_europe, usecols=["name", "ISO numeric", "population"]
    )
    ...
    for country in countries_list:
        ...
        try:
            if run_bedrock:
                ...
                country_population = countrieseurope.loc[
                    country_filter, "population"
                ].iloc[0]
    ...
    logging.info("Concatenating Country files ...")
    # concatenate user country data together and deduplicate across first_names and countries
    output_gen_country_dataframe = pd.concat(
        gen_country_dataframe_list, axis=0, ignore_index=True
    )

    # invert the index ranks and then convert to probability weightings within countries
    def invert_rank_lambda(group):
        group["inverse_rank"] = 1 / (group["rank"] + 1)
        return group

    def conv_proba_lambda(group, power):
        return (group["inverse_rank"].pow(power) + 1) / (
            group["inverse_rank"].pow(power).sum() + 1
        )

    output_gen_country_dataframe["inverse_rank"] = output_gen_country_dataframe.groupby(
        by=["country"], as_index=False, group_keys=False
    ).apply(invert_rank_lambda, include_groups=False)
    if data_point in ("first_names", "last_names"):
        output_gen_country_dataframe["probability"] = (
            output_gen_country_dataframe.groupby(
                by=["country"], as_index=False, group_keys=False
            ).apply(conv_proba_lambda, power=1, include_groups=False)
        )
    elif data_point == "email_domains":
        output_gen_country_dataframe["probability"] = (
            output_gen_country_dataframe.groupby(
                by=["country"], as_index=False, group_keys=False
            ).apply(conv_proba_lambda, power=1.5, include_groups=False)
        )
    else:
        raise ValueError(f"Invalid parameter data_point value {data_point}")
    # sort and deduplicate output data
    sort_dedup_cols = ["country", data_point]
    output_gen_country_dataframe = output_gen_country_dataframe.drop_duplicates(
        subset=sort_dedup_cols
    ).sort_values(by=sort_dedup_cols)
    # write data to disk
    if output_gen_country_dataframe["country"].nunique() == n_countries:
        logging.info(f"Writing reference file: {fpath_dict['fpath']}")
        output_gen_country_dataframe.to_csv(
            fpath_dict["fpath"], index=False, encoding="utf-8"
        )
        logging.info(
            f"output_gen_country_dataframe.shape: {output_gen_country_dataframe.shape}"
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="data_point",
        type=str,
        choices=list(cons.llama_data_point_fpaths.keys()),
        help="String, the data point to generate Bedrock data for.",
    )
    parser.add_argument(
        "--run_bedrock",
        ...
        type=bool,
        default=False,
        help="Boolean, whether to generate data by calling Bedrock",
    )
    ...
    # load aws config
    with open(cons.fpath_aws_session_token) as j:
