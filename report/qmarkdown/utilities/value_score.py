def week_pct_score(group, score_cols=["Successful_size", "Successful_sum"]):
    ...
    group_score = group[score_cols].rank(
        method="average", ascending=True, pct=True, axis=0
    )
    group_score.columns = group_score.columns + "_pct"
    return group_score


def gen_weekly_user_scores(
    group, score_cols=["Successful_size_pct", "Successful_sum_pct"]
):
    """ """
...
def apply_cum_sum(group, score_col="customer_value_score"):
    ...
    customer_value_score_cum_sum = (
        group_sort[score_col].cumsum().rename("customer_value_score_cum_sum")
    )
