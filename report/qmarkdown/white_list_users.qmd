...
from report.qmarkdown.utilities.white_list import gen_white_list
...
user_feat_foath = os.path.join(data_report_dir, 'user_feat_data.csv')
user_feat_data = pd.read_csv(user_feat_foath)
# load customer value score data
customer_value_score_foath = os.path.join(data_report_dir, 'customer_value_score.csv')
...
agg_dict={'Successful_size':'sum', 'Successful_sum':'sum', 'customer_value_score_cum_sum':'mean', 'customer_value_score_cum_sum_pct':'mean'}
high_value_data_agg = high_value_data.groupby(by=groupby_col).agg(agg_dict)
# add total row
total_agg={'Successful_size':'sum', 'Successful_sum':'sum', 'customer_value_score_cum_sum':'mean', 'customer_value_score_cum_sum_pct':'mean'}
...
sub_cols = ['userid', 'customer_value_score_cum_sum_pct', 'transaction_week', 'high_value_user', 'hasE901', 'white_list_users']
...
white_list_users_fpath=os.path.join(data_report_dir, 'white_list_user_data.csv')
